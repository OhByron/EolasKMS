...
<svelte:head>
	<title>{m.page_title_retention()} - {m.admin_title()} - {m.nav_app_title()}</title>
</svelte:head>

<PageHeader title={m.page_title_retention()} description={m.retention_desc()}>
...
{#if loading}
	<p aria-live="polite" class="mt-6 text-muted-foreground">{m.retention_loading()}</p>
{:else if error}
	<div class="mt-6"><ErrorBoundary {error} onRetry={loadPolicies} /></div>
{:else if policies.length === 0}
	<div class="mt-8 text-center">
		<p class="text-lg text-muted-foreground">{m.retention_no_policies()}</p>
		<a href="/admin/retention/new" class="mt-2 inline-block text-sm text-primary underline">{m.retention_create_first()}</a>
	</div>
{:else}
			...
			<a href="/admin/retention/{policy.id}"
				...
				<dl class="mt-3 space-y-1 text-sm">
					<div class="flex justify-between">
						<dt class="text-muted-foreground">{m.retention_card_retention()}</dt>
						<dd class="font-medium">{policy.retentionPeriod}</dd>
					</div>
					{#if policy.reviewInterval}
						<div class="flex justify-between">
							<dt class="text-muted-foreground">{m.retention_card_review()}</dt>
							<dd class="font-medium">{policy.reviewInterval}</dd>
						</div>
					{/if}
					<div class="flex justify-between">
						<dt class="text-muted-foreground">{m.retention_card_on_expiry()}</dt>
